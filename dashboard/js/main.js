document.addEventListener("DOMContentLoaded", function() {
	...
	/*Funciones generales de la interfaz de usuario*/
	let menu_lateral = document.getElementById("menu-lateral");
	let menu_lateral_activo = false;
	let contenedor_general = document.getElementById("contenedor-general");
	let icono_usuario = document.getElementById("icono-usuario");
	...
	icono_usuario.addEventListener("click", function(event) {
	event.preventDefault();

		if(menu_lateral_activo) {
			contenedor_general.style.width = 'calc(100% - var(--size-ancho-menu-lateral))';
			menu_lateral.style.display = 'block';
			menu_lateral_activo = false;
		} else {
			menu_lateral.style.display = 'none';
			contenedor_general.style.width = '100%';
			menu_lateral_activo = true;
		}

		setTimeout(resizeAllGra, 250);
	});

	/*Actualizar redimensión de los gráficos*/
	function resizeAllGra() {
		grafico_principal.resize();
		grafico_primer_mes.resize();
		grafico_segundo_mes.resize();
		grafico_tercer_mes.resize();
	}
